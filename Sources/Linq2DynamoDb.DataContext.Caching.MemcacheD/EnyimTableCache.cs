...
namespace Linq2DynamoDb.DataContext.Caching.MemcacheD
{
    ...
    public partial class EnyimTableCache : ITableCache
    {
        ...
        public async Task<Document> GetSingleEntity(EntityKey entityKey)
        {
            ...
            var wrapper = await this._cacheClient.GetAsync<CacheDocumentWrapper>(entityKeyInCache);
            if (wrapper == null)
            {
        ...
        public async Task PutSingleLoadedEntity(EntityKey entityKey, Document doc)
        {
            string entityKeyInCache = this.GetEntityKeyInCache(entityKey);
            if (entityKeyInCache.Length > MaxKeyLength)
            {
            ...
            await this._cacheClient.StoreAsync(StoreMode.Add, entityKeyInCache, new CacheDocumentWrapper(doc), this._ttl);
        }

        public async Task RemoveEntities(IEnumerable<EntityKey> entities)
        {
            ...
            await Task.WhenAll(
                entityKeysInCache.Select(key => this._cacheClient.DeleteAsync(key)));
        }
        ...
        /// </summary>
        internal async Task LockTable(string lockKey, TimeSpan lockTimeout)
        {
            ...
            while (true)
            {
                ...
                if (await this._cacheClient.StoreAsync(StoreMode.Add, cacheLockKey, cacheLockId))
                {
            ...
            // (this means, that another process crashed), then we should forcibly acquire it

            this.Log("Forcibly acquiring the table lock object {0} after {1} ms of waiting", lockKey, lockTimeout.TotalMilliseconds);

            await this._cacheClient.StoreAsync(StoreMode.Set, cacheLockKey, cacheLockId);
            this._lockIds[lockKey] = cacheLockId;
        }
        ...
        /// </summary>
        internal async Task UnlockTable(string lockKey)
        {
            int lockId;
            ...
            string cacheLockKey = this.GetLockKeyInCache(this._hashKeyValue, lockKey);
            var cacheLockId = await this._cacheClient.GetAsync(cacheLockKey);
            if (cacheLockId == null)
            {
            ...
            await this._cacheClient.DeleteAsync(cacheLockKey);
        }
        ...
        /// </summary>
        private async Task UpdateIndexes(string hashKeyValue, IDictionary<EntityKey, Document> addedEntities, IDictionary<EntityKey, Document> modifiedEntities, ICollection<EntityKey> removedEntities)
        {
            string indexListKeyInCache = this.GetIndexListKeyInCache(hashKeyValue);

            var indexes = await this._cacheClient.GetAsync<TableIndexList>(indexListKeyInCache);
            if (indexes == null)
            {
                await this._cacheClient.DeleteAsync(indexListKeyInCache);
                return;
            }
