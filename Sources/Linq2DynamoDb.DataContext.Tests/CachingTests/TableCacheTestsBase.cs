...
namespace Linq2DynamoDb.DataContext.Tests.CachingTests
{
    public abstract class TableCacheTestsBase
    {
        ...
        [Test]
        public async Task TableCache_SingleEntitiesSavedAndLoaded()
        {
            // filling the cache in
            var books = new Dictionary<EntityKey, Book>
            {
                {new EntityKey(1, Guid.NewGuid()), await BooksHelper.CreateBookAsync(persistToDynamoDb: false)},
                {new EntityKey(Guid.NewGuid(), "2"), await BooksHelper.CreateBookAsync(persistToDynamoDb: false)},
                {new EntityKey(DateTime.Now, 1.23456), await BooksHelper.CreateBookAsync(persistToDynamoDb: false)}
            };

            // some garnish
            this.TableCache.PutSingleLoadedEntity(new EntityKey(0, 0), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
            ...
            // some garnish
            this.TableCache.PutSingleLoadedEntity(new EntityKey("AAAA", "BBBB"), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
        ...
        [Test]
        public async Task TableCache_SingleEntityIsNotLoadedAfterBeingRemoved()
        {
            this._cacheHitCount = 0;
            this._cacheMissCount = 0;
            ...
            // trying twice
            this.TableCache.PutSingleLoadedEntity(key, this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
        ...
        [Test]
        public async Task TableCache_IndexIsCreatedAndThenLoaded()
        {
            var indexBooks = new Dictionary<EntityKey, Book>
            {
                {new EntityKey(Guid.NewGuid(), Guid.NewGuid()), await BooksHelper.CreateBookAsync(persistToDynamoDb: false)},
                {new EntityKey(Guid.NewGuid(), Guid.NewGuid()), await BooksHelper.CreateBookAsync(persistToDynamoDb: false)},
                {new EntityKey(Guid.NewGuid(), Guid.NewGuid()), await BooksHelper.CreateBookAsync(persistToDynamoDb: false)}
            };
        ...
        [Test]
        public async Task TableCache_IndexIsRecreated()
        {
            var conditions = new SearchConditions();
            ...
            using (var indexCreator = this.TableCache.StartCreatingIndex(conditions))
            {
                indexCreator.AddEntityToIndex(new EntityKey(Guid.NewGuid(), Guid.NewGuid()), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
                indexCreator.AddEntityToIndex(new EntityKey(Guid.NewGuid(), Guid.NewGuid()), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
                indexCreator.AddEntityToIndex(new EntityKey(Guid.NewGuid(), Guid.NewGuid()), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
            }

            var indexBooks = new Dictionary<EntityKey, Book>();
            indexBooks[new EntityKey(Guid.NewGuid(), Guid.NewGuid())] = await BooksHelper.CreateBookAsync(persistToDynamoDb: false);
            indexBooks[new EntityKey(Guid.NewGuid(), Guid.NewGuid())] = await BooksHelper.CreateBookAsync(persistToDynamoDb: false);
            indexBooks[new EntityKey(Guid.NewGuid(), Guid.NewGuid())] = await BooksHelper.CreateBookAsync(persistToDynamoDb: false);
        ...
        [Test]
        public async Task TableCache_EmptyIndexIsCreatedAndThenLoaded()
        {
            ...
            // some garnish
            this.TableCache.PutSingleLoadedEntity(new EntityKey(1, 1), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
        ...
        [Test]
        public async Task TableCache_IndexIsDiscardedIfModifiedDuringCreation()
        {
            // creating an empty index
            var conditions = new SearchConditions();
            ...
            using (var indexCreator = this.TableCache.StartCreatingIndex(conditions))
            {
                ...
                (
                    ...
                    {
                        {new EntityKey(Guid.NewGuid(), Guid.NewGuid()), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false))}
                    },
                    new Dictionary<EntityKey, Document>(),
                    new EntityKey[0]
                );
        ...
        [Test]
        public async Task TableCache_IndexIsDiscardedIfEntityIsMissing()
        {
            ...
            using (var indexCreator = this.TableCache.StartCreatingIndex(conditions))
            {
                for (int i = 0; i < bookCount; i++)
                {
                    ...
                    indexCreator.AddEntityToIndex(bookKey, this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
        ...
        [Test]
        public async Task TableCache_IndexIsModifiedSuccessfully()
        {
            ...
            var book1 = await BooksHelper.CreateBookAsync(author: "Mark Twain", lastRentTime: DateTime.MinValue, persistToDynamoDb: false);
            ...
            var book2 = await BooksHelper.CreateBookAsync(author: "Mark Twain", lastRentTime: DateTime.Now, persistToDynamoDb: false);
            ...
            var book3 = await BooksHelper.CreateBookAsync(author: "Mark Twain", lastRentTime: DateTime.Now, persistToDynamoDb: false);
        ...
        [Test]
        public async Task TableCache_EntityModificationIsReflectedInIndexes()
        {
            var book = await BooksHelper.CreateBookAsync(numPages: 150, persistToDynamoDb: false);
            var bookKey = new EntityKey(Guid.NewGuid(), Guid.NewGuid());
        ...
        [Test]
        public async Task TableCache_IndexCreationCancelledIfIndexIsModifiedInParallel()
        {
            var conditions = new SearchConditions();
        ...
        [Test]
        public async Task TableCache_IndexDoesNotResurrectAfterExpiring()
        {
            var conditions = new SearchConditions();
            conditions.AddCondition("Author", new SearchCondition(ScanOperator.NotEqual, Guid.NewGuid().ToString().ToDynamoDbEntry(typeof(string))));
        ...
        [Test]
        public async Task TableCache_IndexIsNotAccessibleWhileBeingCreated()
        {
            var conditions = new SearchConditions();
        ...
        [Test]
        public async Task TableCache_WhenIndexesCreatedInParallelTheLastOneIsSaved()
        {
            var indexBooks = new Dictionary<EntityKey, Book>
            {
                {new EntityKey(Guid.NewGuid(), Guid.NewGuid()), await BooksHelper.CreateBookAsync(persistToDynamoDb: false)},
            ...
            using (var indexCreator = this.TableCache.StartCreatingIndex(conditions))
            {
                ...
                {
                    indexCreator2.AddEntityToIndex(new EntityKey(Guid.NewGuid(), Guid.NewGuid()), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
                    indexCreator2.AddEntityToIndex(new EntityKey(Guid.NewGuid(), Guid.NewGuid()), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false)));
                }
        ...
        [Test]
        public async Task TableCache_LocalChangesAreNeverOverwrittenWhenCreatingAnIndex()
        {
            var bookKey1 = new EntityKey(1, 1);
            var book1 = await BooksHelper.CreateBookAsync(author: "Mark Twain", numPages: 1, persistToDynamoDb: false);
            var bookKey2 = new EntityKey(2, 2);
            var book2 = await BooksHelper.CreateBookAsync(author: "Mark Twain", numPages: 2, persistToDynamoDb: false);
            var book21 = await BooksHelper.CreateBookAsync(author: "Mark Twain", numPages: 21, persistToDynamoDb: false);
            var bookKey3 = new EntityKey(3, 3);
            var book3 = await BooksHelper.CreateBookAsync(author: "Mark Twain", numPages: 3, persistToDynamoDb: false);
            ...
            using (var indexCreator = this.TableCache.StartCreatingIndex(index2))
            {
                ...
                indexCreator.AddEntityToIndex(new EntityKey(Guid.NewGuid(), Guid.NewGuid()), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(author: "Mark Twain", persistToDynamoDb: false)));
                indexCreator.AddEntityToIndex(new EntityKey(Guid.NewGuid(), Guid.NewGuid()), this.ToDocumentConverter(await BooksHelper.CreateBookAsync(author: "Mark Twain", persistToDynamoDb: false)));
        ...
        [Test]
        public async Task TableCache_ManyLargeIndexesAreCreatedAndUpdated()
        {
            ...
            {
                var doc = this.ToDocumentConverter(await BooksHelper.CreateBookAsync(persistToDynamoDb: false));
